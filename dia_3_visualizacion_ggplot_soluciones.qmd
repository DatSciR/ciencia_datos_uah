...
# Gráfico base avanzado 

reto_plot <- ggplot(penguins,aes(x = body_mass_g, y = flipper_length_mm,                         color = species, shape = sex, size = bill_length_mm)) +   
  geom_point(alpha = 0.6, na.rm = TRUE) +   
  coord_cartesian(ylim = c(2500, 5000)) +  # Zoom sin perder datos   
  labs(     
    title = "Condición física de los pingüinos afectados por enfermedad",         
    subtitle = "Relación entre masa corporal, longitud de aleta y tamaño del pico",     x = "Masa corporal (g)",     
  ...
  theme(     
    ...
    )  # Facet grid por sexo y isla 

reto_plot + facet_grid(rows = vars(sex), cols = vars(island))
```
